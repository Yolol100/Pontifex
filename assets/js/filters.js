(function(window, $) {
    ...
    // Consolidated updatePontifexTable function
    PontifexOI.updatePontifexTable = function(page = 1) {
        ...
        // Gecorrigeerde regel: aanroepen van 'planning' in plaats van 'get_planning'
        PontifexOI.apiPost('planning', { filters }).done(function(res) {
            const data = res?.data || res;
            if (!data || !data.planning) return;

            const planning = data.planning || [];
            const cur = data.current_page || 1;
            const tot = data.total_pages || 1;
    ...
    window.PontifexOI.getLanguagesForExam = getLanguagesForExam; 
