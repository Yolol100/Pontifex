(function(window, $) {
	 ...
	 * Update 2026: Alles 21% BTW, geen uitzonderingen voor boeken.
	 */
	function recalcTotal() {
		...
		$('.extra-material-checkbox:checked').each(function() {
			...
			// Update 2026: Alles 21% BTW, ongeacht 'boek' in ID
			vat21_extras += priceExcl * 0.21;
		});
		...
		// Alles naar 21% geschaald
		const finalBaseVat = preciseRound(baseVat * count, 2);
		const finalExtrasVat = preciseRound(vat21_extras * count, 2);

		const vatAll = preciseRound(finalBaseVat + finalExtrasVat, 2);
