...
$order_company = $order['order_company'] ?? ''; // TOEGEVOEGD
$order_vat = $order['order_vat'] ?? ''; // New VAT field
...
$location = $order['location'] ?? ''; // TOEGEVOEGD
$date = $order['date'] ?? ''; // TOEGEVOEGD
$time = $order['time'] ?? ''; // TOEGEVOEGD
...
// Zorg dat $EXTRA_PRODUCTS beschikbaar is (placeholder)
if (!isset($EXTRA_PRODUCTS) || !is_array($EXTRA_PRODUCTS)) {
	$EXTRA_PRODUCTS = [
		'boek-vca-basis-nl' => ['label' => 'VCA Basis Boek (NL)'],
		'e-learning-vca-basis-nl' => ['label' => 'E-Learning VCA Basis (NL)'],
		// Voeg hier eventueel andere benodigde producten toe
	];
}

// Opgeschoonde logica voor $is_weekend_cursus (niet gebruikt in dit template maar behouden)
$is_weekend_cursus =
...
if ($price === '' || $price === null || !isset($order['vat_total'])) {
	// Let op: Afhankelijkheid van \PontifexOI\Helpers\PaymentHelpers
...
// De laatste zichtbare rij krijgt border-bottom: none; anders 1px solid #ddd.
...
<head>
	...
	<style>
		...
		@media only screen and (max-width:600px) {
			...
			/* Stack columns by setting width to 100% and display to block */
			...
			/* Remove internal padding when stacked */
			...
			<tr>
				<td class="header-logo">
					<a href="<?php echo esc_url(home_url('/')); ?>" target="_blank" style="display:inline-block;">
						<img src="https://certipro.nl/wp-content/uploads/2025/07/Logo-1-4.png"
							alt="CertiPro"
							width="120"
					...
					<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:30px;">
						<tr>
							...
							<td class="stack stack-padding-left" width="50%" valign="top" style="padding-left:10px;">
								<?php
								$border_style = 'border-bottom:1px solid #ddd;';
								?>
								...
								<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
									style="background:#f9f9f9; border-radius:6px; border-collapse:collapse; width:100%;">
										...
										<?php else: // FLOW 2: Weekendcursus / directe link (Aangepaste weergave) ?>
											<tr>
												<td style="padding:8px; font-weight:bold; color:#000; <?php echo $border_style; ?>">Examen:</td>
												<td style="padding:8px; color:#333; <?php echo $border_style; ?>">
													<?php 
													// Gebruik een label uit order als beschikbaar, anders fallback logica
										...
										<tr>
											<td style="padding:8px; font-weight:bold; color:#000; <?php echo $border_style; ?>">BTW:</td>
											<td style="padding:8px; color:#333; <?php echo $border_style; ?>">
												<?php
													// Gebruikt nu vat_total als die is ingesteld door de nieuwe fallback, anders vat
													$vat_value = $order['vat_total'] ?? $order['vat'] ?? '';
													echo $vat_value !== '' ? '€' . number_format((float)$vat_value, 2, ',', '.') : '€0,00';
												?>
											</td>
										</tr>
										<tr><td style="padding:8px; font-weight:bold; color:#000; border-bottom:none;">Totaal:</td><td style="padding:8px; color:#333; border-bottom:none;"><?php echo htmlspecialchars($price); ?></td></tr>
								</table>
								</td>
						</tr>
					</table>
