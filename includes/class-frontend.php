...
class Frontend
{
    ...
    public function process_payment()
    {
        ...
        $totals = \PontifexOI\Helpers\PaymentHelpers::calculate_totals_with_vat($order_data); $calculated_total_incl = (float)($totals['incl'] ?? 0.0); $amount_from_frontend = (float) ($_POST['amount'] ?? ($_POST['payment_amount'] ?? 0)); // ✅ GEFIXTE VALIDATIE: Meer tolerante check met uitgebreide logging $difference = abs($amount_from_frontend - $calculated_total_incl); // Log ALTIJD de berekening (ook bij success) error_log('[Pontifex OI PAYMENT][' . self::get_log_context() . '] Bedragvalidatie: Frontend=' . number_format($amount_from_frontend, 2) . ' | Backend=' . number_format($calculated_total_incl, 2) . ' | Verschil=' . number_format($difference, 4) . ' | Kandidaten=' . $order_data['candidate_count'] . ' | Exam=' . $order_data['exam_type'] . ' | Extras=' . json_encode($order_data['extra_options'])); // ✅ Verhoogde tolerantie naar €0.05 (was €0.02) // Dit vangt floating point errors op bij complexe berekeningen met meerdere kandidaten if ($amount_from_frontend <= 0 || $difference > 0.05) { error_log('[Pontifex OI ERROR][' . self::get_log_context() . '] Bedragvalidatie GEFAALD - Te groot verschil of nul bedrag'); error_log('[Pontifex OI DEBUG][' . self::get_log_context() . '] Volledige order_data: ' . json_encode($order_data, JSON_PRETTY_PRINT)); error_log('[Pontifex OI DEBUG][' . self::get_log_context() . '] Herberekende totals: ' . json_encode($totals, JSON_PRETTY_PRINT)); wp_send_json_error([ 'message' => 'Ongeldig bedrag (BTW). Probeer het opnieuw.', 'debug' => WP_DEBUG ? [ 'frontend' => $amount_from_frontend, 'backend' => $calculated_total_incl, 'difference' => $difference ] : null ]); return; }
