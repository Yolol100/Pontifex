...
final class Frontend
{
    ...
    public function enqueue_assets(): void
    {
        ...
        $version = static fn(string $file): int => filemtime($base_path . $file) ?: time();
        ...
        foreach ($js_deps as $handle => $deps) {
            $file = "assets/js/pontifex-oi-{$handle}.js";
    ...
    public function render_planning_shortcode(array $atts = []): string
    {
        ...
        return sprintf(
            ...
            esc_attr(json_encode($_GET, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE))
        );
    }
    ...
    public function process_payment(): never
    {
        ...
        $frontend_amount = (float)($_POST['amount'] ?? ($_POST['payment_amount'] ?? 0));
        ...
        try {
            ...
            set_transient("mollie_payment_id_for_token_{$token}", $payment->id, 3600);
